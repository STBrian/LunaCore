...
namespace Minecraft::Creative {
    inline const gstd::vector<ItemInstance>& InventoryItems = *reinterpret_cast<gstd::vector<ItemInstance>*>(0x00b0d744);

    inline static void addCreativeItem(Item* item, u8 categoryId, s16 position) {
            ItemInstance itemins(item->itemId, 1, 0); // Def values
            itemins.mCategory = categoryId;
            itemins.mNumber = position;
