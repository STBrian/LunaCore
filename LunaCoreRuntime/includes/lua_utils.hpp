...
#define LUAUTILS_SET_ERROR_HANDLER(L)  __lua_emptyerror_handler:                \
    return lua_error(L)



#define LUAUTILS_INIT_TYPEERROR_HANDLER() const char* __typeerrorMsg = "expected type \"%s\" (got \"%s\")"

#define LUAUTILS_SET_TYPEERROR_MESSAGE(msg) __typeerrorMsg = msg



#define LUAUTILS_ERRORF(L, fmt, ...) do {                                       \
...
#define LUAUTILS_CHECKTYPE(L, type, n) do {                                     \
    if (lua_type(L, n) != type) {                                               \
        lua_pushfstring(L, __typeerrorMsg,                                      \
            lua_typename(L, type), luaL_typename(L, n));                        \
        goto __lua_emptyerror_handler;                                          \
    } } while (0)
